...
class ListingDirectoryHelper(context: Context) {
    ...
    {
        val script = "function getListings() { let listings = document.getElementsByClassName('col-lg-3 col-md-3 col-sm-6 col-xs-12 grid-item');let result = [];for(let i = 0; i < listings.length; i++){let obj = new Object();obj.listingId = listings[i].childNodes[0].getAttribute('data-id');obj.listingAddress = listings[i].children[0].getAttribute('data-locations');let imagePath = listings[i].childNodes[0].childNodes[0].childNodes[0].children[1].getAttribute('data-bg-image');obj.imageUrl = imagePath.substring(5, imagePath.length-2);obj.text = listings[i].childNodes[0].getElementsByClassName('case27-primary-text listing-preview-title')[0].innerText;obj.contentUrl = listings[i].childNodes[0].childNodes[0].childNodes[0].getAttribute('href');result[i] = obj;}return JSON.stringify(result); } getListings();"
